...
def loanapprv():
    ...
    def predict():
        ...
        if bulk_flag:
            ...
            details.append(int(dep_e.get()))
            details.append(edu_e.get())
            details.append(slfemp_e.get())
            details.append(float(incm_e.get()))
            details.append(float(coincm_e.get()))
            details.append(float(amt_e.get()))
            t = term_e.get()
            details.append(int(ch_e.get()))
            details.append(area_e.get())
            print(details)
            col = ['Gender', 'Married', 'Dependents',	'Education', 'Self_Employed', 'ApplicantIncome', 'CoapplicantIncome', 'LoanAmount', 'Credit_History', 'Property_Area']
            df_test = pd.DataFrame([details], columns = col)
            p = all.pred_loan_apprv(df_test)
            res_l = Label(main_f,text = "Loan Approved:" , font = ("monoscope" , fontsize['text'] , "bold"),bg = color['frame_bg'] , fg = color['text'])
